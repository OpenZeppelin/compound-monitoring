...
exports.handler = async function (autotaskEvent) {
  ...
  const outerPromises = matchReasons.map(async (reason) => {
    const cTokenMarket = reason.params.cToken;
    if (cTokenMarket.toLowerCase() !== cEtherAddress) {
      return undefined;
    }

    const accountAddress = reason.params.account;
  ...
  const discordPromises = outputs.map(async (output) => {
    ...
    try {
      ...
      console.error(`Error trying to post to Slack: ${error}`);
    }
